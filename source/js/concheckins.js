...
function sortAcceptedContribution()
{
	...
	for (var j = 0; j < years.length; j++)
	{
		...
		for (var i = 0; i < data.length; i++)
		{
			...
			{
				if ((grade_method == "senior3" && data[i].check_class_of == "2027") || (grade_method == "all" && data[i].check_class_of != "2027"))	// 这里就是分类显示了，高三的显示高三通过的，全校的显示高二通过的
				{
					k++;	// 有数据写入
...
async function checkMusic()
{
	...
	else if (music_url.includes(kgmid_format))
	{
		...
		var mid_type = "kgmid";
		addMusicInfoDisplay(mid_type, kgmid, music_url);
		$('.message#murl').html('备用站目前不完全支持添加酷狗音乐链接，建议在<a class="link msg-link" href="https://bjezxkl.pages.dev/?port=2233">主站</a>进行操作');
	}
