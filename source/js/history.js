...
function getDataFromXML(xmlContent, operator, limitationDate, limitationGrade)
{
	...
	for (let i = 0; i < elements.length; i++) {
		...
		{
			case 1:
				if ((c.type == "1" || c.type == "2" || c.type == "3" || c.type == "4" || (c.showname != "" && c.showname != "001钟声1 / 08钟声1" && c.showname != "002钟声2" && c.term != "2014-2015-2-14")) && date_check)
				{
					data.push(c);
				}
				break;
...
function sortMusic()
{
	...
	for (var j = 0; j < years.length; j++)
	{
		...
		for (var i = 0; i < data.length; i++)
		{
			...
			if (date[0] == year)
			{
				if (parseInt(data[i].type) == grade_method_int || data[i].type == "-1/0/1" || data[i].type == "-1/0/2" || data[i].type == "-1/0/3" || data[i].type == "-1/0/4")	// -1是防止parseInt返回NaN
				{
					k++;	// 有数据写入
					if (data[i].mid != "")
					{
...
$(document).on('click', '.grade-wrap', function ()
{
	$('.clear-span.search-month-clear').trigger("click");
	$('.clear-span.search-keyword-clear').trigger("click");
	if ($(this).find('span').hasClass('all') && (grade == '2024' || grade == '2025' || grade == '2026' || grade == '2027'))
	{
