...
function sortSchedulingContribution()
{
	...
	for (var j = 0; j < years.length; j++)
	{
		...
		for (var i = 0; i < data.length; i++)
		{
			...
			{
				if ((grade_method_list == "senior3" && data[i].check_class_of == "2025") || (grade_method_list == "all" && data[i].check_class_of != "2025"))	// 这里就是分类显示了，高三的显示高三通过的，全校的显示高二通过的
				{
					k++;	// 有数据写入
...
//					var state_text = "<span class='state-error'>无版权</span>"
		...
		for (var i = 0; i < scheduled_con_data.length; i++)
		{
			...
			if ((scheduled_con_data[i].date == days[processing_date_index] && scheduled_con_data[i].check_type == "planned") &&
				((scheduled_con_data[i].check_class_of == "2025" && grade_method_plan == "senior3") ||
				 (scheduled_con_data[i].check_class_of != "2025" && grade_method_plan == "all")))	// 在这里新增年级判断语句
			{
